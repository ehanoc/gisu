...
function makeStyles(
  ...
  background='#F9F9F9',
  nodeColor=color.node.normal,
  acceptedNodeColor=color.node.accepted,
  transitionColor=color.transition.normal,
  acceptedTransitionColor=color.transition.accepted
) {
 let styles = {
    wrapper: {
      base: {
...
class StoryGraphView extends Component {
  ...
  handleNodeMouseEnter(d, i, nodes) {
    ...
    d3.selectAll('use.node-control')
      .transition()
      .attr('opacity', 0)

    node.selectAll('use.node-control')
      .transition()
      .attr('opacity', 1)
  }

  handleNodeMouseLeave(d, i, nodes) {
...
StoryGraphView.defaultProps = {
  ...
  renderDefs: (graphView)=>{
    ...
    Object.keys(props.controlTypes).forEach(function(type){
      defIndex += 1
      graphConfigDefs.push(React.cloneElement(props.controlTypes[type].shape, {key: defIndex}))
    })
...
export default Radium(StoryGraphView)
